...
const Player1 = () => {
    ...
    const [rigidBodyPosition, setRigidBodyPosition] = useState([-114, 50, -18.16])
    ...
    useFrame((state) => {
        ...
        direction.subVectors(frontVector, sideVector).normalize().multiplyScalar(10).applyEuler(state.camera.rotation);
        ...
        const cameraQuaternionRotation = camera.getWorldQuaternion(new THREE.Quaternion());
        // console.log('Camera Y rotation : ', camera.rotation.y);
        // console.log({ cameraQuaternionRotation });
        // const cameraEuler = new THREE.Euler().setFromQuaternion(cameraQuaternionRotation);
        headRef.current.setRotation(cameraQuaternionRotation);
        // midBodyRef.current.setRotation(cameraQuaternionRotation)

        if (midBodyRef.current) {
            // const currentQuaternion = midBodyRef.current.quaternion.clone();
            const cameraEuler = new THREE.Euler().setFromQuaternion(cameraQuaternionRotation);
            ...
            // const yAxisQuaternion = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), cameraEuler.y);

            // Combine the current X and Z rotations with the new Y rotation
            // const currentXZ = new THREE.Quaternion().setFromEuler(new THREE.Euler(currentQuaternion.toEuler().x, 0, currentQuaternion.toEuler().z));
            // midBodyRef.current.quaternion.copy(new THREE.Quaternion().multiplyQuaternions(currentXZ, yAxisQuaternion));
            // midBodyRef.current.matrixAutoUpdate = true;
        }
        // legRef.current.setRotation(cameraQuaternionRotation);

        // console.log('Leg Ref Current : ', legRef.current.rotation);
    ...
    return (
        <>
            <group>
                <RigidBody position={[-114, 50, -18.16]} ref={playerRef} lockRotations={true} type="dynamic"
                    ...
                    <CapsuleCollider args={[0.1 / 2, 0.28]} position={[0, 1.40, 0]} ref={headRef} />
                    {/* <CapsuleCollider args={[0.3, 0.35]} position={[0, 0.6, -0.01]} ref={midBodyRef} /> */}
                    <CuboidCollider args={[0.32, 0.5, 0.19]} position={[0, 0.58, 0.012]} ref={midBodyRef} />
                    <CapsuleCollider args={[0.4, 0.3]} position={[0, -0.5, 0]} ref={legRef} />
                    {headRotation && <VoxelCharacter hRotate={headRotation} />}
                </RigidBody>
            </group>
